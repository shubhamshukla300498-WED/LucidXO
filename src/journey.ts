export const worlds=['Living Vault','Gyroid Reef','Crystal Lattice'] as const;
export type World=typeof worlds[number];
export class Journey {
  from=0;to=0;blend=1;age=0;beats=0;private lastOnset=0;
  update(dt:number,selected:number,automatic:boolean,energy:number,onset:number,playing:boolean){
    ...
    if(this.blend<1){this.blend=Math.min(1,this.blend+dt/5);this.lastOnset=onset;return;}
    if(automatic&&energy>.005){this.age+=dt;if(onset>.6&&this.lastOnset<=.6)this.beats++;}
    this.lastOnset=onset;
  }
  get ready(){return this.blend===1&&((this.age>=16&&this.beats>=16)||this.age>=32);}
