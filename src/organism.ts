...
export class Organism {
  ...
  private world:T.Mesh;private worldMaterial:T.ShaderMaterial;private travel=0;private pattern=0;private patternTarget=0;private lastBeat=0;private journey=new Journey();
  ...
  private animate=(now:number)=>{
    ...
    if(s.playing)this.travel+=dt*(.3+s.motion*1.2+f.energy*s.reactivity*.7);
    if(s.playing){if(f.onset>.6&&this.lastBeat<=.6)this.patternTarget+=.7*s.reactivity;this.lastBeat=f.onset;this.pattern+=(this.patternTarget-this.pattern)*(1-Math.exp(-dt/.22));}
    ...
    const w=this.worldMaterial.uniforms;w.uPattern.value=this.pattern;w.uFrom.value=this.journey.from;w.uTo.value=this.journey.to;w.uBlend.value=this.journey.mix;w.uTime.value=this.travel;w.uBass.value=(f.bass*.12+f.kick*.88)*s.reactivity;w.uMid.value=(f.mid*.15+f.snare*.85)*s.reactivity;w.uHigh.value=f.hat*s.reactivity;w.uOnset.value=f.snare*s.reactivity;w.uEnergy.value=s.energy;w.uGlow.value=s.glow;w.uImageMix.value=this.imageTexture!==this.blank?s.imageMix:0;w.uImage.value=this.imageTexture;w.uAspect.value=this.camera.aspect;w.uZoom.value=12.2/this.camera.position.length();w.uDetail.value=s.detail;w.uSeed.value=s.seed;w.uColor.value.setHex(palettes[s.palette].core);this.camera.updateMatrixWorld();w.uCamera.value.copy(this.camera.matrixWorld);
